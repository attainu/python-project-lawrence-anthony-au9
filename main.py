...
pygame.display.set_caption('Simple 2 player Chess')

# Initialize chess variables

# Flag that determines which side's turn it is. If it is even, it is white's turn and if it is odd, it is black's turn
...
def getPromote(win, side_flag, board, fro, to):
    if getType(side_flag, board, fro) == "p":
        if (side_flag == 0 and to[1] == 1) or (side_flag == 1 and to[1] == 8):
            return getChoice(win, side_flag)
